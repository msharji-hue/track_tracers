...
%     black dotted: d = (d1/2)*ln(1+v0^2/(g*d1)) [Katsuragi & Durian 2007]
    ...
    for j = 1:numel(heights)
        hg = heights(j);
        for i = 1:hg.nTrials
            v0_all(end+1) = hg.trials(i).scalars.v0_cm_s;
            d_all(end+1)  = hg.trials(i).scalars.d_final_cm;
        end
    end
    ...
    d1_fit     = fminsearch(@(d1) sum((d_all - kats_model(d1, v0_all)).^2), 10);
    r2_kats    = 1 - sum((d_all - kats_model(d1_fit, v0_all)).^2) / ss_tot;

    % ── Console summary ───────────────────────────────────────────────────
    fprintf('\n── Model fit summary ────────────────────────────\n');
    fprintf('Linear:    d0=%.3f  alpha=%.5f  R2=%.4f\n', d0_fit, alpha_fit, r2_lin);
    fprintf('Katsuragi: d1=%.3f cm            R2=%.4f\n', d1_fit,           r2_kats);
    fprintf('─────────────────────────────────────────────────\n\n');

    % ── Reference curves ──────────────────────────────────────────────────
    v0_line = linspace(80, max(v0_all), 200);
    ...
    % ── Grey dashed: linear empirical fit ────────────────────────────────
    plot(ax, v0_line, d_lin, '--', ...
        'Color',            [0.55 0.55 0.55], ...
        'LineWidth',        1.8, ...
        'HandleVisibility', 'off');

    % ── Black dotted: Katsuragi model ─────────────────────────────────────
    plot(ax, v0_line, d_kats, ':', ...
        'Color',            [0 0 0], ...
        'LineWidth',        2.2, ...
        'HandleVisibility', 'off');

    % ── Mean +- std per height group ──────────────────────────────────────
    for j = 1:numel(heights)
        hg          = heights(j);
        col         = cmap(j,:);
    ...
    % ── Height group legend (right of axes) ───────────────────────────────
    lgd1          = legend(ax, 'show', 'FontSize', 10, 'Box', 'off', ...
                           'Interpreter', 'tex');
    lgd1.Location = 'none';
    lgd1.Position = [0.76 0.40 0.22 0.50];

    % ── Model legend (bottom-right inside axes) — matching paper style ────
    ...
    lgd2 = legend(ax2, 'show', 'FontSize', 11, 'Box', 'on', ...
              'Interpreter', 'tex', ...
              'EdgeColor',   [0.15 0.15 0.15], ...
              'LineWidth',   1.2, ...
              'Color',       [1 1 1]);
    lgd2.Location = 'none';
    lgd2.Position = [0.32 0.18 0.50 0.13];

    % ── Fit annotation (top-left) ─────────────────────────────────────────
    text(ax, 0.03, 0.97, ...
    ...
    exportgraphics(fig, 'd_vs_v0.pdf', 'ContentType', 'vector');
end