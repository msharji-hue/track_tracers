...
%       v_min      - minimum velocity for fit AND display (cm/s), default 0
    ...
    % ── Per-height intercepts (Katsuragi Fig. 3b approach) ───────────────
    % For each height group × depth: F(zi)/m = mean(a+g - v²/d1)
    % d1 is fixed at the shared value above.
    nH                = numel(heights);
    ...
    fitStats.Fz_over_m_se     = nan(nZ,1);   % placeholder — use per-height instead
    ...
    for zi = 1:nZ
        n_valid = sum(isfinite(Fz_per_height(:,zi)));
        fprintf('  z=%.2f cm:  F/m = %.1f cm s^{-2}  (%d height groups)\n', ...
            z_targets(zi), Fz_over_m(zi), n_valid);
    end
    fprintf('-------------------------------------------------------------\n\n');
    ...
    % ── Figure ────────────────────────────────────────────────────────────
    fig = figure('Name','a+g vs v (Katsuragi)','ToolBar','none','MenuBar','none');
    ...
    for zi = 1:nZ
        ...
        % Shared-d1 fit line (parallel, from 0)
        ag_fit_line = Fz_over_m(zi) + v_line.^2 ./ d1_fit;
        plot(ax, v_line, ag_fit_line, 'k--', 'LineWidth', 1.8, ...
            'HandleVisibility', 'off');
    end

    tick_start = ceil(v_min / 50) * 50;   % round up to next 50 (e.g. 40→50)
    set(ax, 'FontSize',14, 'Box','on', 'LineWidth',1.2, ...
        ...
        'YLim',  [0, ag_max_plot]);
    grid(ax,'off');
    xlabel(ax,'$v$  (cm s$^{-1}$)','FontSize',20,'Interpreter','latex','Color',[0 0 0]);
    ylabel(ax,'$a+g$  (cm s$^{-2}$)','FontSize',20,'Interpreter','latex','Color',[0 0 0]);

    lgd = legend(ax,'show','FontSize',11,'Box','on','Interpreter','latex', ...
    ...
    text(ax, 0.97, 0.05, ...
        ...
        'FontAngle','italic','Interpreter','latex','Color',[0.15 0.15 0.15], ...
    ...
    for zi = 1:nZ
        ...
        plot(ax_in, v2_line, ag_fit_line, 'k--', 'LineWidth', 1.4, ...
            'HandleVisibility','off');
    end
    ...
    xlabel(ax_in,'$v^2$  (cm$^2$ s$^{-2}$)', ...
        'FontSize',12,'Interpreter','latex','Color',[0 0 0]);
