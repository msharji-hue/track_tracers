function fig = plot_ag_vs_v2(heights, cmap_depths, z_targets)
...
%       fig - figure handle
    ...
    nZ     = numel(z_targets);
    tol    = 0.05;      % cm — tight depth tolerance
    ag_cap = 12000;     % cm/s^2 — physical cap to remove noisy outliers

    % ── Collect (v, a+g) at each fixed depth across all trials ───────────
    v_at_z  = cell(nZ, 1);
    ag_at_z = cell(nZ, 1);
    ...
    for zi = 1:nZ
        ...
        % Clip v_ref to actual data range at this depth
        ...
        % Fit curve (clipped to data range)
        plot(ax, v_ref, ag_ref, ':', ...
            'Color',     col, ...
            'LineWidth', 2.2, ...
    ...
    set(ax, 'FontSize',   13,  'Box',        'on', ...
            ...
            'TickDir',    'in', ...
            'XLim', [0,   max(cellfun(@max, v_at_z(~cellfun(@isempty,v_at_z))))*1.08], ...
    ...
    lgd = legend(ax, 'show', 'FontSize', 11, 'Box', 'on', ...
                 'Interpreter', 'tex', ...
    ...
    for zi = 1:nZ
        ...
        scatter(ax_in, v2_data, ag_at_z{zi}, 12, col, 'filled', ...
    ...
    grid(ax_in, 'off');
    xlabel(ax_in, '$v^2$  (cm$^2$ s$^{-2}$)', 'FontSize', 11, ...
        'Interpreter', 'latex', 'Color', [0 0 0]);
    ylabel(ax_in, '$a+g$', 'FontSize', 11, ...
        'Interpreter', 'latex', 'Color', [0 0 0]);

    exportgraphics(fig, 'ag_vs_v2.pdf', 'ContentType', 'vector');
end