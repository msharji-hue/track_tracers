function fig = plot_normalized_collapse(heights, cmap)
    ...
    % Reference horizontals — EXACT match to main plot: color, style, linewidth
    yline(ax_in, 1.00, ':', 'Color', [0.05 0.45 0.45], 'LineWidth', 2.4, ...
        'HandleVisibility', 'off');                          % teal dotted    — quadratic drag
    yline(ax_in, 0.50, 'k:', 'LineWidth', 2.8, ...
        'HandleVisibility', 'off');                          % black dotted   — const. drag
    yline(ax_in, 2/3, '-.', 'Color', [0.35 0.35 0.35], 'LineWidth', 2.8, ...
        'HandleVisibility', 'off');                          % grey dash-dot  — linear resist.
    yline(ax_in, alpha_fit, 'k--', 'LineWidth', 3.4, ...
        'HandleVisibility', 'off');                          % black dashed   — global fit

    % Log-linear trend: solid black — distinct from all dashed reference lines
    plot(ax_in, v0ref, alpha_trend, 'k-', 'LineWidth', 1.8, ...
        'HandleVisibility', 'off');

    % Per-height markers: shape from get_height_style, colored fill, black error bars
    ok_idx = find(ok);
    for jj = 1:numel(ok_idx)
        j = ok_idx(jj);
        [~, marker] = get_height_style(heights(j).h_cm);
        errorbar(ax_in, v0vals(j), alpha_j(j), ...
            alpha_j(j)   - alpha_jlo(j), ...
            alpha_jhi(j) - alpha_j(j), ...
            marker, ...
            'Color',           'k', ...          % black error bars
            'MarkerFaceColor', cmap(j,:), ...    % filled with height-group color
    ...
    set(ax_in, 'XScale', 'log', ...
        ...
        'YLim', [0, max(1.05, max(alpha_jhi(ok))*1.05)]);
    grid(ax_in, 'off');
